...
public class ReservationView extends Layout {
    ...
    public ReservationView(Hotel selectedHotel, Room selectedRoom, LocalDate startDate, LocalDate endDate, Reservation existingReservation) {
        ...
        this.add(container);
        initilizeGui(600, 700); // GUI'yi başlatır
        ...
        rezervasyonKaydetButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (!isPriceCalculated) {
                    Helper.showMsg("Fiyat hesaplanmadan rezervasyon yapılamaz!");
                    return;
                }
                ...
                if (areAllFieldsFilled()) {
                    try {
                        ...
                        if (success) {
                            ...
                            JOptionPane.showMessageDialog(null, "Rezervasyon kaydı hatası.", "Hata", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (NumberFormatException ex) {
                        Helper.showMsg("fill");
                    } catch (Exception ex) {
